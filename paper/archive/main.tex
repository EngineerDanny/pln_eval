...
For count prediction, we compare PLN and \texttt{GLMNet(Poisson)} on 20 real microbiome datasets spanning $N = 32$--$18{,}270$ and $D = 24$--$257$, using held-out Poisson deviance under Leave-One-Taxon-Out cross-validation.
...
The primary predictor of the winner is the sample-to-taxon ratio $N/D$, with mean absolute correlation (MAC) as the strongest secondary signal and overdispersion as an additional predictor.
...
For prediction, we compare PLN and \texttt{GLMNet(Poisson)} across 20 real microbiome datasets under a Leave-One-Taxon-Out cross-validation (LOTO-CV) scheme~\cite{agyapong2025cross}, evaluating held-out Poisson deviance across a wide range of sample sizes and dimensionalities.
...
\begin{itemize}
  \item We benchmark PLN against \texttt{GLMNet(Poisson)} for count prediction across 20 real microbiome datasets under a unified Leave-One-Taxon-Out cross-validation~\cite{agyapong2025cross} protocol, quantifying predictive uncertainty through paired deviance comparisons. This provides the first held-out comparison of these two model classes on real data.
  \item We identify $N/D$ as the primary empirical predictor of relative performance, MAC as the strongest secondary signal, and overdispersion as an additional predictor. PLN improves on \texttt{GLMNet(Poisson)} by up to 38\% when $N/D$ is low and inter-taxon dependence is strong, whereas \texttt{GLMNet(Poisson)} dominates when $N \gg D$.
...
Figure~\ref{fig:nd_mac_scatter} shows that the sample-to-taxon ratio $N/D$ is the primary determinant of the dataset-level winner, yielding an AUC of 0.83 on its own.
...
The same result table, including standard errors and paired $p$-values, is also provided as \texttt{out/count\_prediction\_results\_retained20\_with\_stats.csv} in the repository output.
...
This interpretation is consistent with the observed AUC of 0.84 obtained from $N/D$ and MAC together, with overdispersion providing further discriminative power.
...
Computational work was performed on Northern Arizona University's Monsoon high-performance computing cluster~\cite{Buechler2025MonsoonIntro}.
