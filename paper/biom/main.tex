...
\def\ps@headings{\let\@mkboth\markboth
 \def\@oddhead{\Large\hfill{\small\it\@shorttitle}\hfill\hspace{1.5em}%
   \rm\@ddell\hbox to 1.5em{\hss\small\thepage}}
 \def\@oddfoot{}%
 \def\@evenhead{\Large\@ddell\hbox to 1.5em{\small\thepage\hss}\hspace{1.5em}\hfill\small{\it Biometrics, \@month}\ \@pubyear\hfill}
 \def\@evenfoot{}%
...
We study this question using exclusively real microbiome data under a unified held-out evaluation framework.
For count prediction, we compare PLN and \texttt{GLMNet(Poisson)} on 20 real microbiome datasets spanning $N = 32$--$18{,}270$ and $D = 24$--$257$, using held-out Poisson deviance under Leave-One-Taxon-Out cross-validation rather than synthetic or in-sample criteria.
For network inference, we compare PLNNetwork and \texttt{GLMNet(Poisson)} neighbourhood selection on all five publicly available real datasets with experimentally validated microbial interaction truth.
PLN outperforms \texttt{GLMNet(Poisson)} on most count-prediction datasets, with gains up to 38\%.
The primary predictor of the winner is the sample-to-taxon ratio $N/D$, with MAC as the strongest secondary signal and overdispersion as an additional predictor.
...
Taken together, these results provide practical guidance for choosing between latent multivariate count models and penalized Poisson regression in biological count prediction and interaction recovery.
\end{abstract}
...
First, \emph{compositionality}: total read depth (library size) varies across samples by orders of magnitude and carries no biological information; models that ignore this offset confound technical sequencing variation with biological signal~\citep{mcmurdie2014waste}.
...
Poisson PCA has been assessed mainly through simulations that emphasize principal component recovery, robustness, and computational efficiency, with microbiome datasets used as motivating illustrations~\citep{Kenney2021PoissonPCA, Virta2023PoissonPCA}.
These studies evaluate reconstruction error and latent dimension recovery on training data rather than held-out predictive performance, and they do not include penalized Poisson regression as a baseline.
\citet{Chiquet2021} present qualitative applications spanning ordination, regression, network inference, and clustering across several ecological and microbiome datasets, but they do not impose a standardized cross-validation protocol or compare against penalized regression.
Extensions for zero inflation and hierarchical structure compare PLN variants on synthetic and real microbiome data using in-sample criteria such as the variational evidence lower bound, log-likelihood, information criteria, and downstream discrimination or generative performance~\citep{Batardiere2025ZIPLN, Chaussard2025PLNTree}.
...
We evaluated count prediction on a heterogeneous benchmark of 20 publicly available real microbiome datasets.
These datasets span oral, skin, nasal, stool, infant, pediatric, and general gut systems, together with disease-focused collections for colorectal cancer, \textit{Clostridium difficile} infection, HIV, obesity, and type~1 diabetes.
The retained benchmark includes family-, genus-, species-, and OTU-level feature tables.
Across datasets, sample size ranged from $N=32$ to $N=18{,}270$, feature dimension ranged from $D=24$ to $D=257$, and the fraction of zero entries ranged from $30.64\%$ to $93.01\%$.
...
Runtime and memory scaling were profiled using the \texttt{atime} R package on the \citet{Vogtmann2016CRC} stool species dataset.
...
Mean Absolute Correlation (MAC) is the strongest secondary signal and gives an intuitive summary of inter-taxon dependence strength.
Among datasets where \texttt{GLMNet(Poisson)} wins despite low $N/D$, CosteaPI~2017 ($N/D = 1.56$, MAC $= 0.055$) has especially low MAC, consistent with weak community dependence reducing the benefit of PLN's latent layer.
American Gut~1 ($N/D = 2.28$, MAC $= 0.182$) is an outlier where \texttt{GLMNet(Poisson)} wins despite moderate dependence.
Overdispersion is an additional predictor that further separates settings where PLN's latent Gaussian layer is most beneficial.
This matches the model structure: the latent Gaussian layer is most useful when counts depart substantially from the Poisson assumption and when joint dependence among taxa carries predictive signal.
...
The OMM12 example in Figure~S1 of the Supplementary Material illustrates this contrast.
OMM12 is a defined gut community comprising 12 bacterial taxa shown as nodes in Figure~S1.
...
Computational work was performed on Northern Arizona University's Monsoon HPC cluster~\citep{Buechler2025MonsoonIntro}.
