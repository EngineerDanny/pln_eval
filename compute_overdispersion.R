## Compute mean overdispersion index (variance/mean) per dataset
## Only for the 30 datasets remaining after featureless-filter removal
...
## Mean variance/mean ratio across taxa (only taxa with mean > 0)
compute_overdispersion <- function(counts) {
  means <- colMeans(counts)
  vars  <- apply(counts, 2, var)
  keep  <- means > 0
...
for (ds in names(dataset_paths)) {
  ...
  results[[length(results) + 1]] <- data.frame(
    ...
    pln_vs_lasso  = if (nrow(row) > 0) row$pln_vs_lasso_pct else NA,
    mac           = if (nrow(row) > 0) row$mac else NA,
...
fmt <- "%-50s %6s %5s %8s %6s %s\n"
cat(sprintf(fmt, "Dataset", "N/p", "MAC", "Overdisp", "Winner", "PLN vs Lasso"))
cat(paste(rep("-", 100), collapse=""), "\n")
for (i in seq_len(nrow(out))) {
  r <- out[i, ]
  cat(sprintf(fmt, substr(r$dataset, 1, 50), r$N_over_p, r$mac,
              r$overdispersion, r$winner, r$pln_vs_lasso))
}

cat("\nMean OD — PLN wins: ",
    round(mean(out$overdispersion[out$winner == "PLN"]), 2), "\n")
cat("Mean OD — Lasso wins:",
    round(mean(out$overdispersion[out$winner == "Lasso"]), 2), "\n")

write.csv(out, file.path(out_dir, "overdispersion_summary.csv"), row.names = FALSE)
cat("\nSaved to:", file.path(out_dir, "overdispersion_summary.csv"), "\n")
